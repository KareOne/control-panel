...
export default function DomainsPage() {
  const { data, mutate } = useSWR<Domain[]>("/api/domains", fetcher);
  ...
  const save = async () => {
    ...
    try {
      ...
      if (editId) {
        await fetch(`/api/domains/${editId}`, { method: "PUT", headers: { "content-type": "application/json" }, body: JSON.stringify(body) });
      } else {
  ...
  return (
    <div>
      <PageHeader title="Domains & SSL" desc="Monitor domain DNS, SSL certificate expiry, and reverse proxy configuration." />
