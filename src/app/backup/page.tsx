...
function NewBackupForm({ onCreated, initialKind, initialRef }: { onCreated: () => void; initialKind?: BackupTargetKind; initialRef?: string }) {
  ...
  async function submit(e: React.FormEvent) {
    ...
    setErr(null);
    try {
      await api("/api/backup/jobs", "POST", { targetKind: kind, targetRef: ref.trim(), destination: dest });
      setRef("");
      onCreated();
    } catch (ex) {
...
function NewScheduleForm({ onCreated }: { onCreated: () => void }) {
  ...
  async function submit(e: React.FormEvent) {
    ...
    setErr(null);
    try {
      await api("/api/backup/schedules", "POST", {
        name: name.trim(),
        targetKind: kind,
        targetRef: ref.trim(),
        cronExpr: cron.trim(),
...
function ScheduleCard({
  ...
  async function handleDelete() {
    ...
    setDeleting(true);
    try {
      await api(`/api/backup/schedules/${schedule.id}`, "DELETE");
...
function JobsTab({
  ...
  const { data: jobs, mutate, isLoading } = useSWR<BackupJob[]>("/api/backup/jobs", fetcher);
...
function SchedulesTab() {
  const { data: schedules, mutate, isLoading } = useSWR<BackupSchedule[]>("/api/backup/schedules", fetcher);
  const [showForm, setShowForm] = useState(false);

  async function handleToggle(id: string, enabled: boolean) {
    await api(`/api/backup/schedules/${id}`, "PATCH", { enabled });
    mutate();
  }
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: "16px" }}>
      ...
      {showForm && (
        <NewScheduleForm onCreated={() => { setShowForm(false); mutate(); }} />
      )}
...
function DiscoveredTab({ onBackupNow }: { onBackupNow: (kind: BackupTargetKind, ref: string) => void }) {
  const { data, isLoading, mutate } = useSWR<DiscoveredData>("/api/backup/discovered", fetcher);
  ...
  if (!data) return <EmptyState msg="Could not reach Docker socket." />;
  ...
  return (
    <div>
      ...
      <div className="p-6 space-y-6">
        ...
        {/* Tab content */}
        {tab === "jobs"       && <JobsTab prefilledKind={quickBackup?.kind} prefilledRef={quickBackup?.ref} onConsumed={() => setQuickBackup(null)} />}
        {tab === "schedules"  && <SchedulesTab />}
        {tab === "discovered" && <DiscoveredTab onBackupNow={handleBackupNow} />}
      </div>
    </div>
  );
}
