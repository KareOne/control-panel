...
function DockerDaemonSection({ lang }: { lang: "en" | "fa" }) {
  const { data, error, isLoading, mutate } = useSWR<DockerDaemonData>(
    "/api/server/docker-daemon",
    fetcher,
  ...
  const apply = async (restartDocker: boolean) => {
    ...
    setResult(null);
    try {
      const res = await fetch("/api/server/docker-daemon", {
        method: "POST",
...
function SSHSection({ lang }: { lang: "en" | "fa" }) {
  const { data, error, isLoading, mutate } = useSWR<SSHData>("/api/server/ssh", fetcher, {
...
function FirewallSection({ lang }: { lang: "en" | "fa" }) {
  ...
  const post = async (body: Record<string, unknown>) => {
    ...
    setResult(null);
    try {
      const res = await fetch("/api/server/firewall", {
        method: "POST",
        headers: { "content-type": "application/json" },
...
function PortAuditSection({ lang }: { lang: "en" | "fa" }) {
...
function NetworkSection({ lang }: { lang: "en" | "fa" }) {
...
function SystemServicesSection({ lang }: { lang: "en" | "fa" }) {
...
function OSSection({ lang }: { lang: "en" | "fa" }) {
...
function ResourcesSection({ lang }: { lang: "en" | "fa" }) {
...
function TimeSection({ lang }: { lang: "en" | "fa" }) {
  ...
  return (
    <div dir={rtl ? "rtl" : "ltr"}>
      ...
      <div className="space-y-6 p-6">
        <DockerDaemonSection lang={lang} />
        <SSHSection lang={lang} />
        <FirewallSection lang={lang} />
        <PortAuditSection lang={lang} />
        <NetworkSection lang={lang} />
        <SystemServicesSection lang={lang} />
        <OSSection lang={lang} />
