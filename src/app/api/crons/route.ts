...
async function readCrontabs(): Promise<CronEntry[]> {
  const all: CronEntry[] = [];

  // Root's personal crontab
  try {
    const { stdout } = await hostExec("crontab -l 2>/dev/null || true");
    if (stdout.trim()) {
  ...
  try {
    const { stdout } = await hostExec("cat /etc/crontab 2>/dev/null || true");
    if (stdout.trim()) {
  ...
  try {
    const { stdout: files } = await hostExec(
      "ls /etc/cron.d/ 2>/dev/null | grep -v '.placeholder' || true"
    );
    for (const file of files.split("\n").map((f) => f.trim()).filter(Boolean)) {
      try {
        const { stdout } = await hostExec(`cat /etc/cron.d/${file} 2>/dev/null || true`);
        if (stdout.trim()) {
...
export const GET = handler(async (req: NextRequest) => {
  ...
  const entries = await readCrontabs();
  return json({ entries });
});
...
export const POST = handler(async (req: NextRequest) => {
  ...
  const { stdout: existing } = await hostExec("crontab -l 2>/dev/null || true");
  const updated = (existing.trimEnd() + "\n" + newLine + "\n").trimStart();
  const escaped = updated.replace(/'/g, "'\\''");
  await hostExec(`echo '${escaped}' | crontab -`);
