...
async function checkFirewall(): Promise<BaselineCheck> {
  try {
...
async function checkRootSsh(): Promise<BaselineCheck> {
  try {
    const { stdout } = await hostExec("sshd -T 2>/dev/null | grep \"^permitrootlogin\"", 5000);
...
async function checkDockerLogs(): Promise<BaselineCheck> {
  try {
    const { stdout } = await hostExec("cat /etc/docker/daemon.json 2>/dev/null || echo {}", 5000);
...
async function checkFail2ban(): Promise<BaselineCheck> {
  try {
    const { stdout } = await hostExec(
...
async function checkSshPort(): Promise<BaselineCheck> {
  try {
    const { stdout } = await hostExec("sshd -T 2>/dev/null | grep \"^port \"", 5000);
...
async function checkDockerSocket(): Promise<BaselineCheck> {
  try {
    const { stdout } = await hostExec("stat -c \"%a\" /var/run/docker.sock 2>/dev/null", 5000);
...
async function checkPasswordAuth(): Promise<BaselineCheck> {
  try {
    const { stdout } = await hostExec("sshd -T 2>/dev/null | grep \"^passwordauthentication\"", 5000);
    const value = stdout.trim().split(/\s+/)[1]?.toLowerCase() ?? "";
    if (value === "no") {
